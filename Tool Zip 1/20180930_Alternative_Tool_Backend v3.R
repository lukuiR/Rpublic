#########     WORKFORCE FORECASTING TOOL: BACK END     ##########

# This is the backend code for the workforce forecasting tool. It should not be touched, 
# except to set the working directory as outlined below. There is no need to have this file
# open when running the application

##### Setting the working directory #####

# The working directory is the folder in which the tool is stored. Replace the file path
# below with the path of the folder. 

#Note that all slashes should be forward slashes, i.e. "/" not "\"

#working_directory <- "C:/Users/charlie-kershaw/Documents/Saudi/STC/Workforce Model/R/Tool"

##### Setting the name of the import file #####

# This tells the code the name of the csv of the imported data. This csv file should be stored in the
# same folder as the tool - the folder specified above.

data_import_file <- "20180924_Data_Import_Tool_v3.csv"
...
# Setting working directory. Note this must be commented out if uploading to a server
...
Blue1 <- "#002C77"  #Darkest blue
Blue2 <- "#006D9E"  #Darker blue
Blue3 <- "#00A8C8"  #Lighter blue
Blue5 <- "#A6E2EF"  #Lightest blue

# Data Import Section -----------------------------------------------------

# Importing raw data
raw_data <- read_csv(data_import_file)
colnames <- colnames(raw_data)


data_sectioninfo <- raw_data %>% select(unique_identifier,l0,l1,l2,l3,l4,level,segment)
data_sectioninfo[c("l1","l2","l3","l4")][is.na(data_sectioninfo[c("l1","l2","l3","l4")])] <- "None"
data_sectioninfo[c("l1","l2","l3","l4")][data_sectioninfo[c("l1","l2","l3","l4")]=="None"] <- "None"
...
# Renaming autorlcm to aut
for(x in c("min","mid","max")){
  for(y in c("Y1","Y2","Y3")){
    outputcol <- paste("aut",x,y,sep="_")
    inputcolautorlcm <- as.name(paste("autorlcm",x,y,sep="_"))
    b_output_forecast_automation <- b_output_forecast_automation %>%
      rowwise() %>%
      mutate(!!outputcol := !!inputcolautorlcm)
  }
}
