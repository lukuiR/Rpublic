              ...
              fluidRow({
                box(
                  ...
                  radioButtons("radio_t2rsss", label = NULL,
                               choiceNames = list("Conservative (BB)","Base Case (BE)", "Optimistic (BD)"
                               ),
                               choiceValues = list(
                                 "rs", "r_opt_s","r_crvt_s"
...
server <- function(input, output,session) {
  ...
  nms =sum(aoc()$subt)-(sum(sign(aoc()$not_map)*aoc()$subt) + sum(sign(aoc()$Specialist)*aoc()$subt) + sum(sign(aoc()$Support)*aoc()$subt) + sum(sign(aoc()$Critical)*aoc()$subt))
  revenue <-c(sum(aoc()$total)-sum(aoc()$subt), sum(sign(aoc()$Core)*aoc()$subt),sum(sign(aoc()$Specialist)*aoc()$subt),sum(sign(aoc()$Support)*aoc()$subt),sum(sign(aoc()$Critical)*aoc()$subt), nms)
  ...
  revenue <-c(sum(aoc()$Core)-sum(sign(aoc()$Core)*aoc()$subt)  ,  sum(aoc()$Specialist)-sum(sign(aoc()$Specialist)*aoc()$subt)  ,  sum(aoc()$Support)-sum(sign(aoc()$Support)*aoc()$subt)  ,  sum(aoc()$Critical)-sum(sign(aoc()$Critical)*aoc()$subt)  ,        nm-nms)
  cat=c(rep(c('1.Core','2.Specialist','3.Support','4.Critical','5.Not Map'),1))
  ...
  plot_ly(data, x = ~x, y = ~revenue, type = 'bar',name='Demand',
    ...
    layout(title = 'Headcount', margin = list(b = 100),
           ...
           barmode = 'stack',
           showlegend = TRUE)%>%
    add_annotations(text = sum(aoc()$total),
