                  ...
                  tabBox(
                    ...
                    tabPanel("Supply",
                               fluidRow({
                                 ...
                                 column(4,
                                        knobInput(
                                          inputId = "t2sk11",label = "Year 1 NHR:",thickness=0.2, width=100, height=100,
      ...
      tabItem(tabName = "tab3",
              ...
              fluidRow(
                box(
                  title =HTML( '<font color=#ffffff> <font size="5">Department Selection</font></font>')
                  , status = "primary", solidHeader = TRUE,width = '100%',collapsible = TRUE,collapsed=TRUE,
                  column(4,h6("1.SABIC All"),actionButton("sall2","1.SABIC All",width = '100%')),
                  column(4, uiOutput('uityp3')),column(4,uiOutput('uirg3')),column(4,uiOutput('uiname3')),
                  column(4, uiOutput('uidiv3')),column(4,uiOutput('uijob3')),column(4,uiOutput('uisjob3'))
                  #https://stackoverflow.com/questions/51355878/how-to-text-wrap-choices-from-a-pickerinput-if-the-length-of-the-choices-are-lo
                )
              ),#filters
              fluidRow({
...
server <- function(input, output,session) {
  ...
  }#new
  {
    ...
    output$uityp3 = renderUI({
      pickerInput("uipType3",label=HTML('<font color=#454545>2.Function/BU Type</font>'),choices=unique(db$`CF/SBU/AFF`[order(db$`CF/SBU/AFF`)]),
                  selected=unique(db$`CF/SBU/AFF`),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3"
                  ),  multiple = TRUE)
    })
    output$uirg3 = renderUI({
      pickerInput("uiprg3",label=HTML('<font color=#454545>3.Region</font>'),choices=outrg3(),
                  selected=outrg3(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3"
                  ),  multiple = TRUE)
    })
    ...
    output$uiname3 = renderUI({
      pickerInput("uipname3",label=HTML('<font color=#454545>4.Function/Affiliate Name</font>'),choices=outnm3(),
                  selected= outnm3(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
    output$uidiv3 = renderUI({
      pickerInput("uipdiv3",label=HTML('<font color=#454545>5.Org. Structure</font>'),choices=outdiv3(),
                  selected= outdiv3(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
    output$uijob3 = renderUI({
      pickerInput("uipjob3",label=HTML('<font color=#454545>6.Job Family</font>'),choices=outjob3(),
                  selected= outjob3(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
    output$uisjob3 = renderUI({
      pickerInput("uipsjob3",label=HTML('<font color=#454545>7.Job Sub Family</font>'),choices=outsjob3(),
                  selected= outsjob3(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
  ...
  p <-add_trace(p,
    ...
    add_annotations(text = round(sum(aoc()$sray23)-sum(aoc()$fte5),0),bgcolor='rgba(255, 255, 255, 1.0)',bordercolor='rgba(1, 1, 1, 1.0)',
  ...
  p <-add_lines(p, x=c('6.Business (R&M)(-)','7.Automation<br>Reduction') , y= c(data4n$revenue[1], data4n$revenue[1]), name = "",
                line = list(color= 'black', widthh=0.7, dash="dash"), showlegend=FALSE)
  p <-add_lines(p, x=c('7.Automation<br>Reduction','8.Capability<br>Enhancement') , y= c(sum(data5$revenue), sum(data5$revenue)), name = "",
  ...
  output$plot425<- renderPlotly({
    ...
    data <- data.frame(x,y)
    #The default order will be alphabetized unless specified as below:
    plot_ly(data, labels = ~x, values  = ~y, type = 'pie')%>%
      layout(#title = '',
             showlegend = T)
  ...
  output$plot426<- renderPlotly({
    ...
    #The default order will be alphabetized unless specified as below:
    plot_ly(data, x = ~x, y = ~y, type = 'bar',  color = ~cat)%>%
