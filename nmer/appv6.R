      ...
      tabItem(tabName = "tab2",
              ...
              fluidRow({
                box(
                  ...
                  h2('should those tab be here or just drop down filters?'),
                  tabBox(
...
server <- function(input, output,session) {
  ...
  }#old sample
  {
    ...
    output$uityp = renderUI({
      pickerInput("uipType",label=HTML('<font color=#454545>Function/BU Type</font>'),choices=unique(db$`CF/SBU/AFF`[order(db$`CF/SBU/AFF`)]),
                  selected=unique(db$`CF/SBU/AFF`),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3"
                  ),  multiple = TRUE)
    })
    output$uirg = renderUI({
      pickerInput("uiprg",label=HTML('<font color=#454545>Region</font>'),choices=outrg(),
                  selected=outrg(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3"
                  ),  multiple = TRUE)
    })
    ...
    output$uiname = renderUI({
      pickerInput("uipname",label=HTML('<font color=#454545>Function/Affiliate Name</font>'),choices=outnm(),
                selected= outnm(),
                options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
      })
    output$uidiv = renderUI({
      pickerInput("uipdiv",label=HTML('<font color=#454545>Org. Structure</font>'),choices=outdiv(),
                  selected= outdiv(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
    output$uijob = renderUI({
      pickerInput("uipjob",label=HTML('<font color=#454545>Job Family</font>'),choices=outjob(),
                  selected= outjob(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
    output$uisjob = renderUI({
      pickerInput("uipsjob",label=HTML('<font color=#454545>Job Sub Family</font>'),choices=outsjob(),
                  selected= outsjob(),
                  options = list( `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3" ),  multiple = TRUE)
    })
