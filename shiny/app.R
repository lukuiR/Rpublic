...
# Tab 1 Struktura taby i filry plus
# Tab 2 add sesion tab drill 
# Tab 3 alert login
# Tab 4 table plus edit row
# Tab 5 filtry fajne widgety
# Tab 6 plotly i mapy
# Tab 7 deploy?
# Tab n logi
...
#https://www.r-bloggers.com/announcing-the-1st-shiny-contest/
#https://github.com/bborgesr/erum2018/tree/master/app
...
sidebar <- dashboardSidebar(
  ...
  sidebarMenu(id = "sidebar_menu",
              ...
              menuItem("Bonus", tabName = "bonus", icon = icon("star")),
              hr(), 
    ...
    tabItem("bonus",
            fluidRow(
              helpText("A space to dynamically create value boxes corresponding",
                       "to an aggregate summary of the data by subcategory for the selected", 
                       "period of time."),
              h4("Bunus, generator of your shiny app style colours."))
            )
  )
)
...
server <- function(input, output, session) { 
  ...
  output$menuItem_specific_ui <- renderUI({
    ...
    else if (input$sidebar_menu == "bonus") {
      tagList(
        actionButton("create_box", "Create new box", class = "color_btn"),
        actionLink("remove_boxes", "Delete dynamic boxes")
      )
               ...
               column(4, 
                      ...
                      valueBox(width = NULL,
                               ...
                               subtitle = glue("{input$studio_function} over {input$studio_subcategory} ", 
                                               "for {direction} {input$studio_amount} e"), 
