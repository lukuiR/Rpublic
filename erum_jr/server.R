...
shinyServer(function(input, output, session) {
 ...
 ###map
  ...
  cff1=read.csv("girl.csv", header = TRUE, sep=",",colClasses=c("NULL",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA))
  ...
  ######graph
  output$zzz1<- renderText(zzz <- as.character( paste(cff1[cff1$lon==input$map_marker_click$lng,'lon'], input$map_marker_click$lng))
...
###############2
  ...
  output$zzz11<- renderText(as.character( paste( input$kraj,is.null(input$kraj),  aa2[aa2$continent %in% input$cont,3], cff12()[cff12()$lon==input$map2_marker_click$lng,'lon'], input$map2_marker_click$lng
                                                )))
